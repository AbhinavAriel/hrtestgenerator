...
// ── Result pill — handles Rejected / Passed / Failed ──────────────────────────
function ResultPill({ isPassed, isRejected, cancellationReason }: {
  isPassed?: boolean
  isRejected?: boolean
  ...
  if (isRejected) {
    return (
      <span
        title={cancellationReason ?? "Rejected"}
        className="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold bg-orange-100 text-orange-700 cursor-default"
      >
        Rejected
      </span>
    )
...
export default function HrTableRow({ row, onEdit, onDelete, onRowClick }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Result — shows Rejected / Passed / Failed */}
      <td className="p-4">
        {isSubmitted ? (
          <ResultPill
